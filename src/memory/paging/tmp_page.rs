use crate::memory::{Frame, FrameAllocator};

use super::{
    table::{Level1, Table},
    ActivePageTable, Page, VirtAddr,
};

pub struct TemporaryPage {
    page: Page,
    allocator: TinyAllocator,
}

impl TemporaryPage {
    /// Creates a new temporary page.
    pub fn new<A>(page: Page, allocator: &mut A) -> TemporaryPage
    where
        A: FrameAllocator,
    {
        TemporaryPage {
            page,
            allocator: TinyAllocator::new(allocator),
        }
    }

    /// Maps the temporary page to the given frame in the active table and returns its start address.
    pub fn map(&mut self, frame: Frame, active_table: &mut ActivePageTable) -> VirtAddr {
        use super::entry::EntryFlags;

        assert!(
            active_table.translate_page(self.page).is_none(),
            "temporary page is already mapped"
        );
        active_table.map_to(self.page, frame, EntryFlags::WRITABLE, &mut self.allocator);
        self.page.start()
    }

    /// Unmaps the temporary page in the active table.
    pub fn unmap(&mut self, active_table: &mut ActivePageTable) {
        active_table.unmap(self.page, &mut self.allocator);
    }

    /// Maps the temporary page to the given page table frame in the active page table.
    /// Returns a reference to the now mapped table.
    pub fn map_table_frame(
        &mut self,
        frame: Frame,
        active_table: &mut ActivePageTable,
    ) -> &mut Table<Level1> {
        // SAFETY: returned address in self.map will always be a valid page table
        unsafe { &mut *(self.map(frame, active_table) as *mut Table<Level1>) }
    }
}

struct TinyAllocator([Option<Frame>; 3]);

impl FrameAllocator for TinyAllocator {
    fn kalloc_frame(&mut self) -> Option<Frame> {
        for frame in &mut self.0 {
            if frame.is_some() {
                return frame.take();
            }
        }
        None
    }

    fn kfree_frame(&mut self, frame: Frame) {
        for maybe_frame in &mut self.0 {
            if maybe_frame.is_none() {
                *maybe_frame = Some(frame);
                return;
            }
        }
        panic!("Tiny allocator already full!");
    }
}

impl TinyAllocator {
    fn new<A>(allocator: &mut A) -> TinyAllocator
    where
        A: FrameAllocator,
    {
        let mut f = || allocator.kalloc_frame();
        let frames = [f(), f(), f()];
        TinyAllocator(frames)
    }
}
